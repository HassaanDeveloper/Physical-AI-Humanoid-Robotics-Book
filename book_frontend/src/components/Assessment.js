...
const Assessment = ({ title, questions, showAnswers = true }) => {
  ...
  return (
    <div className={styles.assessmentContainer}>
      <div className={styles.assessmentHeader}>
        <h3 className={styles.assessmentTitle}>{title}</h3>
      </div>
          ...
          <div key={qIndex} className={styles.questionContainer}>
            ...
            <div className={styles.answersContainer}>
              {question.answers.map((answer, aIndex) => (
                <div
                  key={aIndex}
                  className={`${styles.answerOption} ${getAnswerClass(qIndex, aIndex, answer.isCorrect)} ${selectedAnswers[qIndex] === aIndex ? styles.selectedAnswer : ''}`}
